...
class FetchProductsUseCaseTest {
    ...
    private val useCase = FetchProductsUseCase(repository)

    @Before
    fun setUp() {

        val product = ProductDataFactory.makeProductDto().mapperToMovieView()
        given { repository.fetchProducts(-1) }.willReturn(Either.right(listOf(product)))
    }
    ...
    fun `should get data from repository`() {
        ...
        verify(repository).fetchProducts(-1)
